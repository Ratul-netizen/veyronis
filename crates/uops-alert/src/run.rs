    ...
    F: Future<Output = ()> + Send,
{
    ...
    loop {
        ...
        if since_reload >= RELOAD {
            ...
            settle_quiet_incidents(&store).await;
...
/// in production does. Production is [`run`]: a wheel dispatching one rule at
...
/// when an alert resolves. It is called on the reload cadence because that is
/// already the per-tenant beat of [`run`].
async fn settle_quiet_incidents(store: &PgStore) {
